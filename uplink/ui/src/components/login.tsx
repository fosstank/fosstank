...
export default function LoginPanel({ isOpen, onClose, onLoginSuccess }: LoginPanelProps) {
    ...
    const handleForgotPassword = async () => {
        if (isForgotPassword) {
            ...
            try {
                setIsLoading(true);
                await pb.collection('users').requestPasswordReset(formData.username);
        ...
        >
            <div className="w-96 flex flex-col gap-6">
                <h2 className="text-xl font-semibold text-zinc-100">
                    {isForgotPassword ? 'Reset Password' : (isLogin ? 'Log In' : 'Create Account')}
                </h2>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div>
                        <label htmlFor="username" className="block text-sm font-medium text-zinc-300 mb-2">
                            {isForgotPassword ? "Email Address" : (isLogin ? "Username or Email" : "Username")}
                        </label>
                        <input
                            id="username"
                            name="username"
                            ...
                            placeholder={isForgotPassword ? "Enter your email address" : (isLogin ? "Enter your username or email" : "Choose a username")}
                        />
                    </div>
